...
const updateCoupon = async (req, res) => {
  try {
    ...
    if (req.body.validUntil) req.body.validUntil = new Date(req.body.validUntil)
...
const getCouponStats = async (req, res) => {
  try {
    ...
    const stats = {
      ...
      totalDiscountGiven: coupon.usageHistory.reduce((total, usage) => total + usage.discountAmount, 0),
...
const getActiveCoupons = async (req, res) => {
  try {
    ...
    }).select("code description type value minimumOrderAmount maximumDiscountAmount")
    ...
    for (const coupon of coupons) {
      ...
      const userUsageCount = coupon.usageHistory.filter((usage) => usage.user.toString() === req.user.id).length
